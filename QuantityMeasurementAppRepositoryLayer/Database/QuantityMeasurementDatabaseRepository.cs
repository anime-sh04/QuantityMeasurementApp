...
{
    /// <summary>
    /// UC16: SQL Server (SSMS) implementation of IQuantityMeasurementRepository.
    /// Uses ADO.NET with parameterized queries to prevent SQL injection.
    /// Implements connection pooling via SqlConnection's built-in pooling.
    /// </summary>
    public class QuantityMeasurementDatabaseRepository : IQuantityMeasurementRepository
    {
        ...
        public void Save(QuantityMeasurementEntity entity)
        {
            ...
            const string insertSql = @"
                INSERT INTO QuantityMeasurementEntities
                    (OperationType, MeasurementType, FirstValue, FirstUnit,
                     SecondValue, SecondUnit, Result, CreatedAt)
                OUTPUT INSERTED.Id
                VALUES
                    (@OperationType, @MeasurementType, @FirstValue, @FirstUnit,
                     @SecondValue, @SecondUnit, @Result, @CreatedAt)";
            ...
            try
            {
                ...
                _logger.LogInformation("Saved entity Id={Id} ({Op}/{Type}) to database.", entity.Id, entity.OperationType, entity.MeasurementType);
        ...
        public List<QuantityMeasurementEntity> GetAllMeasurements()
        {
            const string sql = @"
                SELECT Id, OperationType, MeasurementType,
                       FirstValue, FirstUnit, SecondValue, SecondUnit, Result, CreatedAt
            ...
            {
                throw new DatabaseException("Failed to retrieve all measurements.", "GetAll", ex);
            }
        }
        ...
        public List<QuantityMeasurementEntity> GetMeasurementsByOperation(string operationType)
        {
            ...
            {
                throw new DatabaseException($"Failed to retrieve by operation '{operationType}'.", "GetByOperation", ex);
            }
        }
        ...
        public List<QuantityMeasurementEntity> GetMeasurementsByType(string measurementType)
        {
            if (string.IsNullOrWhiteSpace(measurementType))
                throw new ArgumentException("measurementType cannot be null or empty.", nameof(measurementType));
            ...
            {
                throw new DatabaseException($"Failed to retrieve by type '{measurementType}'.", "GetByType", ex);
            }
        }
        ...
        public int GetTotalCount()
        {
            ...
            {
                throw new DatabaseException("Failed to get total count.", "GetTotalCount", ex);
            }
        }
        ...
        public void DeleteAll()
        {
            try
            {
                ...
                _logger.LogInformation("All measurements deleted from database.");
            }
            catch (SqlException ex)
            {
                throw new DatabaseException("Failed to delete all measurements.", "DeleteAll", ex);
            }
        }
        ...
        {
            return $"Active connections (approx): {_activeConnections} | " +
                   $"ADO.NET built-in connection pooling is active for: {MaskConnectionString(_connectionString)}";
        }
        ...
        private void ExecuteNonQuery(string sql, SqlConnection? existingConn = null)
        {
            bool ownsConnection = existingConn == null;
            var conn = existingConn ?? OpenConnection();
            try
            {
                using var cmd = new SqlCommand(sql, conn);
                cmd.ExecuteNonQuery();
